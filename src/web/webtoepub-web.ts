...
async function parsePage(url: string): Promise<{ title: string; html: string }> {
...
export async function convertWebToEpub(url: string): Promise<{ title: string; chapters: EpubChapter[] }> {
  const first = await parsePage(url)
  const list = extractChapterList(first.html, url)
  let chapters: EpubChapter[]
  if (list.length >= 2) {
    const slice = list.slice(0, 50)
    chapters = await Promise.all(slice.map(async (c) => ({ title: c.title, html: (await parsePage(c.url)).html })))
  } else {
...
function opfXml(opts: { title: string; author?: string; chapters: EpubChapter[] }): string {
  const manifest = opts.chapters.map((_, i) => `<item id="chapter${i + 1}" href="chapter${i + 1}.xhtml" media-type="application/xhtml+xml"/>`).join('\n    ')
...
function ncxXml(opts: { title: string; chapters: EpubChapter[] }): string {
  const points = opts.chapters
    .map((c, i) => `<navPoint id="nav${i + 1}" playOrder="${i + 1}"><navLabel><text>${escapeXml(c.title)}</text></navLabel><content src="chapter${i + 1}.xhtml"/></navPoint>`)
    .join('\n    ')
  return `<?xml version="1.0" encoding="utf-8"?>
...
function xhtml(title: string, bodyHtml: string): string {
  return `<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml"><head><title>${escapeXml(title)}</title></head><body><h1>${escapeXml(title)}</h1>${bodyHtml}</body></html>`
}
