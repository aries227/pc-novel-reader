...
describe('renderLibrary', () => {
  ...
  it('可按书名搜索书架', async () => {
    ...
    const items: LibraryItem[] = [
      { meta: { id: 'a', title: '猫武士', author: 'A', format: 'txt', addedAt: 1 }, bookmarks: [] },
      { meta: { id: 'b', title: '哈利波特', author: 'B', format: 'txt', addedAt: 1 }, bookmarks: [] }
    ]
    ;(window.reader.library as { list: typeof window.reader.library.list }).list = async () => items
    const container = document.createElement('div')
    await renderLibrary(container, () => undefined)
    const input = container.querySelector('.lib-search') as HTMLInputElement
    input.value = '猫'
    input.dispatchEvent(new Event('input'))
    expect(container.querySelectorAll('.book-card').length).toBe(1)
    expect(container.querySelector('.book-title')?.textContent).toContain('猫')
  })
})
