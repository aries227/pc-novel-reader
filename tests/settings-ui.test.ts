...
function mockReader(): void {
  ;(window as unknown as { reader: ReaderApi }).reader = {
    ...
    settings: {
      ...
      set: async (patch) => ({ theme: 'sepia', fontSize: 18, lineHeight: 1.9, fontFamily: 'system', backgroundOpacity: 0.8, mode: 'paged', uploadPortMode: 'random', maxUploadMb: 100, sourceCacheLimit: 50, translateBaseUrl: 'https://api.deepseek.com', translateModel: 'deepseek-chat', translateTarget: '英文', aiProviders: [{ id: 'deepseek', name: 'DeepSeek', baseUrl: 'https://api.deepseek.com', apiKey: '', models: ['deepseek-chat'] }], aiDefaults: { translateProviderId: 'deepseek', translateModel: 'deepseek-chat', quizProviderId: 'deepseek', quizModel: 'deepseek-chat', quizCount: 4, quizDifficulty: '通用' }, examColors: { enabled: true, colors: {}, enabledTags: {} }, shortcuts: { next: 'ArrowRight,PageDown,space', prev: 'ArrowLeft,PageUp', back: 'Escape' }, ...patch }),
...
describe('openSettingsModal', () => {
  ...
  it('可关闭考试词自动着色', async () => {
    ...
    const container = document.createElement('div')
    await openSettingsModal(container)
    const cb = container.querySelector('[data-exam="enabled"]') as HTMLInputElement
    cb.checked = false
    cb.dispatchEvent(new Event('change'))
    await new Promise((r) => setTimeout(r, 0))
    const patch = [...patches].reverse().find((p) => (p as { examColors?: unknown }).examColors) as { examColors: { enabled: boolean } }
    expect(patch.examColors.enabled).toBe(false)
  })
  it('可自定义快捷键', async () => {
    mockReader()
    const patches: unknown[] = []
    ...
    const container = document.createElement('div')
    await openSettingsModal(container)
    const input = container.querySelector('[data-shortcut="next"]') as HTMLInputElement
    input.value = 'ArrowDown'
    input.dispatchEvent(new Event('change'))
    await new Promise((r) => setTimeout(r, 0))
    const patch = [...patches].reverse().find((p) => (p as { shortcuts?: unknown }).shortcuts) as { shortcuts: { next: string } }
    expect(patch.shortcuts.next).toBe('ArrowDown')
  })
  it('可单独开关某个考试级别着色', async () => {
    mockReader()
    const patches: unknown[] = []
    const orig = window.reader.settings.set
    ;(window.reader.settings as { set: typeof orig }).set = async (patch) => {
    ...
    const container = document.createElement('div')
    await openSettingsModal(container)
    const cb = container.querySelector('[data-exam-tag="cet6"]') as HTMLInputElement
    expect(cb.checked).toBe(true)
    cb.checked = false
    cb.dispatchEvent(new Event('change'))
    await new Promise((r) => setTimeout(r, 0))
    const patch = [...patches].reverse().find((p) => (p as { examColors?: unknown }).examColors) as { examColors: { enabledTags: Record<string, boolean> } }
    expect(patch.examColors.enabledTags.cet6).toBe(false)
  })
})
